...
def send_message():
    ...
    try:
        with connection.cursor() as cursor:
            sql = "INSERT INTO ChatroomMessages (chatroom_id, sent_by, message, timestamp) VALUES (%s, %s, %s, %s)"
            cursor.execute(sql, (room_id, profile_id, message, datetime.datetime.now()))
        connection.commit()
    finally:
        connection.close()
...
def upload():
    ...
    if not profileID or not accesstoken:
        abort(400, description="Both profile_id and access_token are required.")
    ...
    try:
        response = requests.post("http://file_service/upload", files = files)
        ...
        if response.status_code == 200:
            ...
            data = response.json()
            return jsonify({"error": f"Failed to Upload but reached other side {file.filename}"})
    except Exception as e:
        print(f"Error forwarding: {e}")
        return jsonify({"error": f"Failed to Upload: {e}"})
