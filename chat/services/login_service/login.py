...
def logout():
    ...
    if not accesstoken:
        return jsonify({"Error": "No Authentication Token"}), 400
    
    invalidate_response = requests.post("http://auth_service/invalidate_token", json={"token": accesstoken})

    if invalidate_response.status_code == 200:
        return redirect(url_for('login'))
    else:
        return render_template('base.html', app_message = "Log Out Failed")
